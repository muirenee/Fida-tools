...
public class CloudMediaSync {
    ...
    private void pushMaintenance(String workspaceId,Result result)throws Exception{
        List<AppDatabase.Row> pending=db.rows("SELECT q.*,m.remote_uuid FROM sync_queue q LEFT JOIN sync_metadata m ON m.entity_type=q.entity_type AND m.entity_id=q.entity_id WHERE q.entity_type='maintenance_log' ORDER BY q.changed_at,q.id",null);
        for(AppDatabase.Row q:pending){long id=longValue(q.s("entity_id"));if(id<=0)continue;if("delete".equals(q.s("operation"))){String remote=q.s("remote_uuid");if(!remote.isEmpty()){client.update("maintenance_logs","id=eq."+remote+"&workspace_id=eq."+workspaceId,new JSONObject().put("deleted_at",java.time.Instant.now().toString()));result.recordsPushed++;}db.markDeletionSynced("maintenance_log",id,remote);continue;}AppDatabase.Row item=db.one("SELECT * FROM maintenance_logs WHERE id=?",new String[]{String.valueOf(id)});if(item.id()==0)continue;
            long assetId=longValue(item.s("asset_id")),jobId=longValue(item.s("job_id"));if(assetId<=0)continue;
            String remote=db.ensureRemoteUuid("maintenance_log",id);JSONObject o=new JSONObject().put("id",remote).put("workspace_id",workspaceId).put("asset_id",db.ensureRemoteUuid("asset",assetId)).put("deleted_at",JSONObject.NULL);
            if(jobId>0)o.put("job_id",db.ensureRemoteUuid("job",jobId));else o.put("job_id",JSONObject.NULL);
            o.put("service_date",item.s("service_date")).put("notes",item.s("notes"));putNullable(o,"next_service",item.s("next_service"));
    ...
    private void pullMaintenance(String workspaceId,Result result)throws Exception{
        JSONArray rows=client.select("maintenance_logs","select=*&workspace_id=eq."+workspaceId+"&order=service_date.asc");SQLiteDatabase d=db.getWritableDatabase();
        for(int i=0;i<rows.length();i++){JSONObject o=rows.getJSONObject(i);String remote=o.optString("id","");if(remote.isEmpty())continue;long local=db.localIdForRemote("maintenance_log",remote);if(isDeleted(o)){if(local>0&&!db.applyRemoteDeletion("maintenance_log",remote))continue;if(local>0)result.recordsPulled++;continue;}if(local>0&&db.hasPendingSync("maintenance_log",local)){db.recordSyncConflict("maintenance_log",local,remote,"Cloud maintenance update deferred because this device has unsynced edits");continue;}long assetId=db.localIdForRemote("asset",nullable(o,"asset_id"));if(assetId<=0)continue;long jobId=db.localIdForRemote("job",nullable(o,"job_id"));ContentValues v=new ContentValues();v.put("asset_id",assetId);if(jobId>0)v.put("job_id",jobId);else v.putNull("job_id");v.put("service_date",nullable(o,"service_date"));v.put("notes",nullable(o,"notes"));v.put("next_service",nullable(o,"next_service"));
            ...
            db.bindRemoteUuid("maintenance_log",local,remote);d.delete("sync_queue","entity_type='maintenance_log' AND entity_id=?",new String[]{String.valueOf(local)});db.resolveSyncConflict("maintenance_log",local);result.recordsPulled++;
        }
    }

    private void pushJobPhotos(String workspaceId,Result result)throws Exception{
        List<AppDatabase.Row> pending=db.rows("SELECT q.*,m.remote_uuid FROM sync_queue q LEFT JOIN sync_metadata m ON m.entity_type=q.entity_type AND m.entity_id=q.entity_id WHERE q.entity_type='job_photo' ORDER BY q.changed_at,q.id",null);
        for(AppDatabase.Row q:pending){long photoId=longValue(q.s("entity_id"));if(photoId<=0)continue;if("delete".equals(q.s("operation"))){String remote=q.s("remote_uuid");if(!remote.isEmpty()){JSONArray old=client.select("job_photos","select=storage_path&id=eq."+remote+"&workspace_id=eq."+workspaceId);String storage=old.length()>0?nullable(old.getJSONObject(0),"storage_path"):"";client.update("job_photos","id=eq."+remote+"&workspace_id=eq."+workspaceId,new JSONObject().put("deleted_at",java.time.Instant.now().toString()));if(!storage.isEmpty()){try{client.deleteObject(BUCKET,storage);}catch(Exception ignored){}}result.recordsPushed++;}db.markDeletionSynced("job_photo",photoId,remote);continue;}AppDatabase.Row p=db.one("SELECT * FROM job_photos WHERE id=?",new String[]{String.valueOf(photoId)});if(p.id()==0)continue;long jobId=longValue(p.s("job_id"));if(jobId<=0)continue;
            ...
            JSONObject o=new JSONObject().put("id",remotePhoto).put("workspace_id",workspaceId).put("job_id",remoteJob).put("storage_path",storage).put("caption",p.s("caption")).put("deleted_at",JSONObject.NULL);client.upsert("job_photos","id",o);db.markEntitySynced("job_photo",photoId,remotePhoto);result.recordsPushed++;
        }
    }

    private void pullJobPhotos(String workspaceId,Result result)throws Exception{
        JSONArray rows=client.select("job_photos","select=*&workspace_id=eq."+workspaceId+"&order=created_at.asc");SQLiteDatabase d=db.getWritableDatabase();
        for(int i=0;i<rows.length();i++){JSONObject o=rows.getJSONObject(i);String remote=o.optString("id","");if(remote.isEmpty())continue;long local=db.localIdForRemote("job_photo",remote);if(isDeleted(o)){if(local>0){AppDatabase.Row old=db.one("SELECT uri FROM job_photos WHERE id=?",new String[]{String.valueOf(local)});if(db.hasPendingSync("job_photo",local)){db.recordSyncConflict("job_photo",local,remote,"Cloud photo deletion deferred because this device has an unsynced photo");continue;}deleteDownloadedFile(old.s("uri"));if(db.applyRemoteDeletion("job_photo",remote))result.recordsPulled++;}continue;}if(local>0&&db.hasPendingSync("job_photo",local)){db.recordSyncConflict("job_photo",local,remote,"Cloud photo update deferred because this device has an unsynced photo");continue;}String storage=nullable(o,"storage_path");if(storage.isEmpty())continue;long jobId=db.localIdForRemote("job",nullable(o,"job_id"));if(jobId<=0)continue;AppDatabase.Row old=local>0?db.one("SELECT * FROM job_photos WHERE id=?",new String[]{String.valueOf(local)}):new AppDatabase.Row();String current=old.s("uri");
            ...
            db.bindRemoteUuid("job_photo",local,remote);d.delete("sync_queue","entity_type='job_photo' AND entity_id=?",new String[]{String.valueOf(local)});db.resolveSyncConflict("job_photo",local);result.recordsPulled++;
        }
    }

    private void pushSignatures(String workspaceId,Result result)throws Exception{
        List<AppDatabase.Row> pending=db.rows("SELECT q.*,m.remote_uuid FROM sync_queue q LEFT JOIN sync_metadata m ON m.entity_type=q.entity_type AND m.entity_id=q.entity_id WHERE q.entity_type='job_signature' ORDER BY q.changed_at,q.id",null);
        for(AppDatabase.Row q:pending){long jobId=longValue(q.s("entity_id"));if(jobId<=0)continue;String remoteJob=db.one("SELECT remote_uuid FROM sync_metadata WHERE entity_type='job' AND entity_id=?",new String[]{String.valueOf(jobId)}).s("remote_uuid"),sigUuid=q.s("remote_uuid");if(sigUuid.isEmpty())sigUuid=db.ensureRemoteUuid("job_signature",jobId);if("delete".equals(q.s("operation"))){String old=prefs.getString("signature_remote_path_"+jobId,"");if(!remoteJob.isEmpty()){if(old.isEmpty()){JSONArray a=client.select("jobs","select=signature_path&id=eq."+remoteJob+"&workspace_id=eq."+workspaceId);if(a.length()>0)old=nullable(a.getJSONObject(0),"signature_path");}client.update("jobs","id=eq."+remoteJob+"&workspace_id=eq."+workspaceId,new JSONObject().put("signature_path",JSONObject.NULL).put("customer_name_signed",""));if(!old.isEmpty()){try{client.deleteObject(BUCKET,old);}catch(Exception ignored){}}result.recordsPushed++;}prefs.edit().remove("signature_remote_path_"+jobId).apply();db.markDeletionSynced("job_signature",jobId,sigUuid);continue;}AppDatabase.Row j=db.one("SELECT * FROM jobs WHERE id=?",new String[]{String.valueOf(jobId)});if(j.id()==0)continue;if(remoteJob.isEmpty())remoteJob=db.ensureRemoteUuid("job",jobId);String local=j.s("signature_path");
            if(local.isEmpty()||!new File(local).isFile()){String old=prefs.getString("signature_remote_path_"+jobId,"");if(old.isEmpty()){JSONArray a=client.select("jobs","select=signature_path&id=eq."+remoteJob+"&workspace_id=eq."+workspaceId);if(a.length()>0)old=nullable(a.getJSONObject(0),"signature_path");}client.update("jobs","id=eq."+remoteJob,new JSONObject().put("signature_path",JSONObject.NULL));if(!old.isEmpty()){try{client.deleteObject(BUCKET,old);}catch(Exception ignored){}}db.markEntitySynced("job_signature",jobId,sigUuid);prefs.edit().remove("signature_remote_path_"+jobId).apply();result.recordsPushed++;continue;}
    ...
    private void pullSignatures(String workspaceId,Result result)throws Exception{
        ...
        for(int i=0;i<rows.length();i++){JSONObject o=rows.getJSONObject(i);String remoteJob=o.optString("id",""),storage=nullable(o,"signature_path");if(remoteJob.isEmpty())continue;long jobId=db.localIdForRemote("job",remoteJob);if(jobId<=0)continue;if(hasPending("job_signature",jobId)){db.recordSyncConflict("job_signature",jobId,remoteJob,"Cloud signature change deferred because this device has an unsynced signature");continue;}AppDatabase.Row local=db.one("SELECT signature_path FROM jobs WHERE id=?",new String[]{String.valueOf(jobId)});String knownPath=prefs.getString("signature_remote_path_"+jobId,"");
            if(storage.isEmpty()){if(!knownPath.isEmpty()||!local.s("signature_path").isEmpty()){deleteDownloadedFile(local.s("signature_path"));ContentValues v=new ContentValues();v.putNull("signature_path");v.put("customer_name_signed","");v.put("updated_at",db.now());d.update("jobs",v,"id=?",new String[]{String.valueOf(jobId)});prefs.edit().remove("signature_remote_path_"+jobId).apply();result.recordsPulled++;}continue;}
            String sigUuid=filenameToken(storage),known=db.one("SELECT remote_uuid FROM sync_metadata WHERE entity_type='job_signature' AND entity_id=?",new String[]{String.valueOf(jobId)}).s("remote_uuid");
            if(!sigUuid.equals(known)||local.s("signature_path").isEmpty()||!new File(local.s("signature_path")).isFile()||!storage.equals(knownPath)){byte[] bytes=client.downloadObject(BUCKET,storage);File f=cloudFile(workspaceId,"signatures",remoteJob,extensionFromPath(storage,"png"));write(f,bytes);ContentValues v=new ContentValues();v.put("signature_path",f.getAbsolutePath());v.put("customer_name_signed",nullable(o,"customer_name_signed"));v.put("updated_at",db.now());d.update("jobs",v,"id=?",new String[]{String.valueOf(jobId)});if(!sigUuid.isEmpty())db.bindRemoteUuid("job_signature",jobId,sigUuid);prefs.edit().putString("signature_remote_path_"+jobId,storage).apply();result.filesDownloaded++;}
            db.resolveSyncConflict("job_signature",jobId);result.recordsPulled++;
        }
    }
    ...
    private void syncBrandingLogo(String workspaceId,boolean canManage,Result result)throws Exception{
        ...
        if(canManage&&(dirty||(remotePath.isEmpty()&&logo.isFile()&&known.isEmpty()))){String old=remotePath;if(logo.isFile()){byte[] bytes=readFile(logo);String storage=workspaceId+"/branding/logo-"+sha(bytes).substring(0,16)+".png";client.uploadObject(BUCKET,storage,bytes,"image/png");client.update("branding_settings","workspace_id=eq."+workspaceId,new JSONObject().put("logo_storage_path",storage));remotePath=storage;result.filesUploaded++;if(!old.isEmpty()&&!old.equals(storage)){try{client.deleteObject(BUCKET,old);}catch(Exception ignored){}}}
    ...
    private boolean hasPending(String type,long id){return db.hasPendingSync(type,id);}
    private boolean isDeleted(JSONObject o){return o!=null&&!o.isNull("deleted_at")&&!o.optString("deleted_at","").isEmpty();}
    private void deleteDownloadedFile(String ref){try{if(ref==null||ref.isEmpty())return;File f;if(ref.startsWith("file://"))f=new File(Uri.parse(ref).getPath());else f=new File(ref);String cloud=new File(context.getFilesDir(),"cloud_media").getCanonicalPath();if(f.isFile()&&f.getCanonicalPath().startsWith(cloud+File.separator))f.delete();}catch(Exception ignored){}}
