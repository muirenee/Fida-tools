...
public class AppDatabase extends SQLiteOpenHelper {
    ...
    @Override public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, report_no TEXT NOT NULL UNIQUE, customer_id INTEGER, site_id INTEGER, asset_id INTEGER, title TEXT NOT NULL, problem TEXT, diagnosis TEXT, work_done TEXT, parts TEXT, technician TEXT, technician_id INTEGER, priority TEXT DEFAULT 'Normal', status TEXT DEFAULT 'Open', job_date TEXT NOT NULL, next_service TEXT, service_started_at_ms INTEGER DEFAULT 0, service_completed_at_ms INTEGER DEFAULT 0, service_duration_minutes INTEGER DEFAULT 0, service_sessions_json TEXT NOT NULL DEFAULT '[]', signature_path TEXT, customer_name_signed TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE SET NULL, FOREIGN KEY(site_id) REFERENCES sites(id) ON DELETE SET NULL, FOREIGN KEY(asset_id) REFERENCES assets(id) ON DELETE SET NULL)");
    ...
    public List<Row> jobsFiltered(String search,String status,String priority,String technician,String fromDate,String toDate) {
        ...
        if (JobStatusFilter.ACTIVE.equals(status)) { sql.append(" AND ").append(JobStatusFilter.activeSql("j")); }
        else if (status != null && !status.isEmpty() && !status.equals("All")) { sql.append(" AND j.status=?"); args.add(status); }
    ...
    public long visibleOpenJobCount(String userUuid,boolean seeAll){if(seeAll)return count("jobs",JobStatusFilter.ACTIVE_SQL,null);return countSql("SELECT COUNT(*) FROM jobs j WHERE "+JobStatusFilter.activeSql("j")+" AND EXISTS (SELECT 1 FROM technicians t WHERE t.id=j.technician_id AND t.user_uuid=?)",new String[]{userUuid==null?"":userUuid});}
    ...
    private JSONArray normalizedServiceSessions(Row job){
