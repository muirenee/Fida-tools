...
# Dashboard quick actions: technicians get job action, managers also get customer action, viewers read-only.
s = replace_once(s,
...
# Customer and site master-data controls are manager-only.
s = replace_once(s,
...
# Replace More with a role-aware menu.
s = regex_once(s,
...
# Direct manager guards protect against stale screens/intents as well as hiding controls.
for sig, feature in [
...
# Viewer cannot create or edit service work.
s = replace_once(s,
...
# Asset detail: read for all members, master-data edit only managers, job action for field workers.
...
# Simplified invite onboarding. A deep-link carries the code; account creation/sign-in then joins automatically.
...
# Make local company/workspace profile editable only for Owner/Admin.
...
# When signed in but not yet bound, only managers get create-workspace; invitation-code fallback remains for everyone.
s = replace_once(s,
...
# Insert invite onboarding helpers before showAccountWorkspace.
...
# Invitation dialog message now reflects automatic email/deep-link onboarding.
...
# --- AppDatabase: discard stale manager-only queues on non-manager devices ---
...
# --- Cloud sync: non-managers never push master-data mutations ---
